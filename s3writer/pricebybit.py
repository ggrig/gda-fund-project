...
class PriceBybit(PriceBase):
    ...
    def orderbook_200_lob_action_data(self, json_data, lob_action_name:str):
        retval = []
        ...
        for entry in lob_actions:
            ...
            'quote_no': entry['id'],
            'end_of_event': False,
            'side': self.get_side_index(entry['side']),
    ...
    def trade_lob_action_data(self, json_data):
        retval = []

        for entry in json_data['data']:
            event = {
            'event_timestamp': entry["timestamp"],
            'event_no': entry['cross_seq'],
            'quote_no': entry['cross_seq'],
            'end_of_event': True,
            'side': self.get_side_index(entry['side']),
            'lob_action': 'trade',
            'original_order_id': entry['trade_id'],
            'order_id': entry['trade_id'],
            'size': entry['size'],
            'old_size': 0,
            'price': float(entry["price"]),
            'old_price': 0,
    ...
    def process_json_data(self, topic:str, json_data):
        ...
        if TOPIC_BYBIT_INSURANCE    == topic:
            ...
            retval += self.orderbook_200_lob_action_data(json_data, 'delete')
            retval += self.orderbook_200_lob_action_data(json_data, 'insert')
            retval += self.orderbook_200_lob_action_data(json_data, 'update')

        elif TOPIC_BYBIT_TRADE      == topic and self.verify_trade_structure(json_data):
            retval += self.trade_lob_action_data(json_data)
